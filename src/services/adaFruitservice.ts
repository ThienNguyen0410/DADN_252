class AdafruitService {
  ...
  private readonly AIO_KEY = import.meta.env.VITE_AIO_KEY || '';
  private readonly USERNAME = import.meta.env.VITE_USERNAME || '';
  ...
  async getHumidity() {
    const res = await fetch(`${this.BASE_URL}/humidity/data`, { headers: this.getHeaders() });
    return await res.json();
  }

  async getTemperature() {
    const res = await fetch(`${this.BASE_URL}/temperature/data`, { headers: this.getHeaders() });
    return await res.json();
  }
